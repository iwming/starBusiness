...
- (void)observe:(id)object info:(nullable _FBKVOInfo *)info
{
  ...
  // add observer  --- 将self 和 object 简历绑定关系 实际监控者!!!!!!!
...
- (void)unobserve:(id)object info:(nullable _FBKVOInfo *)info
{
  ...
  info->_state = _FBKVOInfoStateNotObserving;//别忘记状态修改
}
...
- (void)dealloc
{
  NSLog(@"will release all observer");
  [self unobserveAll];
...
- (void)_observe:(id)object info:(_FBKVOInfo *)info
{
 ...
 //查询Map中对应被观察着的info集合(key、block)，因为该object可以有多个属性被观察的
  NSMutableSet *infos = [_objectInfosMap objectForKey:object];

  // check for info existence
  _FBKVOInfo *existingInfo = [infos member:info];//判断此次要被观察的info是否已经被观察
  if (nil != existingInfo) {
    // observation info already exists; do not observe it again

    // unlock and return
    pthread_mutex_unlock(&_lock); //如果已经被观察了则解锁退出
    return;
  }

  // lazilly create set of infos  如果该Object还没有任何属性被观察，即infos集合为空，则新建并把info插入
  if (nil == infos) {
    infos = [NSMutableSet set];//建立集合，
    [_objectInfosMap setObject:infos forKey:object];//插入进去，
  }

  // add info and oberve
  [infos addObject:info];//出入info到infos集合
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(FBKVONotificationBlock)block
{
  ...
  // create info ---单纯保存kvo各信息
  _FBKVOInfo *info = [[_FBKVOInfo alloc] initWithController:self keyPath:keyPath options:options block:block];

  // observe object with info
  [self _observe:object info:info];//将被观察的object 和 info 建立联系
}
